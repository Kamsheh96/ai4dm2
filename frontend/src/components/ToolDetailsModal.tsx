  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-300 font-bold">{value}/{max}</span>
      </div>
...
const CostIndicator: React.FC<{ cost: string }> = ({ cost }) => {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-sm text-gray-200 font-semibold">Cost</span>
        <span className="text-sm text-gray-300 font-bold">{level.label}</span>
      </div>
      <div className="h-3 bg-gray-700 rounded-full overflow-hidden shadow-inner">
        <div
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[60] overflow-hidden animate-fade-in">
      <div className="flex h-full items-center justify-center p-2 sm:p-4">
        <div className="bg-gray-800 rounded-2xl sm:rounded-3xl shadow-glow w-full max-w-6xl h-full sm:h-[96vh] flex flex-col border border-gray-700 animate-scale-in overflow-hidden">
          {/* Header */}
          <div className="p-6 border-b border-gray-700 flex-shrink-0 bg-gradient-to-br from-gray-800 via-gray-800 to-gray-850">
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-start space-x-4 flex-1">
                ...
                <div className="flex-1">
                  <h2 className="text-2xl sm:text-3xl font-bold text-white mb-2">{tool.name}</h2>
                  <p className="text-base text-gray-300 leading-relaxed">{details.fullDescription}</p>
                  <div className="flex flex-wrap items-center gap-2 mt-4">
                    ...
                    <span className="text-sm text-gray-300 font-medium px-3 py-1.5 bg-gray-700 rounded-full border border-gray-600">
                      {tool.complexity}
                    </span>
                    <span className="text-sm text-gray-300 font-medium px-3 py-1.5 bg-gray-700 rounded-full border border-gray-600">
                      {tool.estimatedTime}
              ...
              <button
                onClick={onClose}
                className="p-3 min-h-[44px] min-w-[44px] hover:bg-gray-700 hover:shadow-soft rounded-2xl transition-all duration-200 hover:scale-110 active:scale-95 touch-manipulation flex-shrink-0 ml-4"
          ...
          {/* Content - Scrollable */}
          <div className="flex-1 overflow-y-auto p-6 sm:p-8 bg-gradient-to-br from-gray-900 via-gray-850 to-gray-900">
            <div className="max-w-5xl mx-auto space-y-8">
                  ...
                  <ul className="space-y-3">
                    {details.keyFeatures.map((feature, i) => (
                      <li key={i} className="flex items-start space-x-3">
                        <span className="w-6 h-6 rounded-lg bg-gradient-to-br from-primary-900/50 to-secondary-900/50 flex items-center justify-center flex-shrink-0 mt-0.5 border border-primary-700">
                ...
                <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6">
                  ...
                  <ul className="space-y-3">
                    {details.useCases.map((useCase, i) => (
                      <li key={i} className="flex items-start space-x-3">
                        <span className="w-6 h-6 rounded-lg bg-gradient-to-br from-success-900/50 to-success-800/50 flex items-center justify-center flex-shrink-0 mt-0.5 border border-success-700">
              ...
              {/* Integrations */}
              <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-white mb-4">Integrations & Compatibility</h3>
                <div className="flex flex-wrap gap-2">
                  {details.integrations.map((integration, i) => (
                    <span
                      key={i}
                      className="text-sm px-4 py-2 bg-gray-700 text-gray-200 rounded-full font-semibold border border-gray-600 hover:scale-105 transition-transform duration-200"
                    >
                      {integration}
                    </span>
                  ))}
              ...
              {details.alternatives.length > 0 && (
                <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6">
                  <h3 className="text-lg font-bold text-white mb-4">Similar Tools</h3>
                  <div className="flex flex-wrap gap-2">
                    {details.alternatives.map((alt, i) => (
                      <span
                        key={i}
                        className="text-sm px-4 py-2 bg-primary-900/30 text-primary-300 rounded-full font-semibold border border-primary-700 hover:scale-105 transition-transform duration-200"
                      >
                        {alt}
                      </span>
                    ))}
          ...
          {/* Footer - Actions */}
          <div className="p-6 border-t border-gray-700 bg-gradient-to-br from-gray-800 to-gray-850 flex-shrink-0">
            <div className="flex items-center justify-between max-w-5xl mx-auto">
              ...
              <div className="flex space-x-3">
                <button
                  onClick={onClose}
                  className="px-6 py-3 min-h-[44px] bg-gray-700 text-gray-200 rounded-2xl font-semibold border border-gray-600 hover:bg-gray-600 hover:shadow-soft hover:scale-105 transition-all duration-200"
                >
                  Close
                </button>
                <button
